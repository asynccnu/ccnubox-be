...
func InitGRPCxKratosServer(departmentServer *grpc.DepartmentServiceServer, ecli *clientv3.Client, l logger.Logger, cfg *conf.TransConf) grpcx.Server {
	server := kgrpc.NewServer(
		kgrpc.Address(cfg.Grpc.Server.Addr),
		kgrpc.Middleware(recovery.Recovery(),
			LoggingMiddleware(l)),
	)
	departmentServer.Register(server)
