...
func wireApp(string2 string, confServer *conf.Server, confData *conf.Data, confRegistry *conf.Registry, logger log.Logger, writer io.Writer) (*APP, func(), error) {
	...
	classroomJSONData := data.NewClassroomJSONData()
	freeClassroomSvc := service.NewFreeClassroomSvc(freeClassroomBiz, classroomJSONData)
	...
	classroomJSONSvc := service.NewClassroomJSONSvc(classroomJSONData)
	httpServer := server.NewHTTPServer(confServer, selectionUploader, classroomJSONSvc, logger)
