...
func (c *Crawler2) GetClassInfosForUndergraduate(ctx context.Context, stuID, year, semester, cookie string) ([]*model.ClassInfoBO, []*model.StudentCourseBO, int, error) {
	// 获取代理并将其存入请求的上下文
	...
	for _, info := range infos {
		scs = append(scs, &model.StudentCourseBO{
			StuID:           stuID,
			ClaID:           info.ID,
			Year:            year,
			Semester:        semester,
...
func (c *Crawler2) GetClassInfoForGraduateStudent(ctx context.Context, stuID, year, semester, cookie string) ([]*model.ClassInfoBO, []*model.StudentCourseBO, int, error) {
	// 获取代理并将其存入请求的上下文
	proxyURL := c.pg.GetProxy(ctx)
	...
	if err != nil {
		...
		return nil, nil, -1, errorx.Errorf("crawler.crawler2.GetClassInfoForGraduateStudent new request failed: %w", err)
	}
	req.Header = http.Header{
		"Cookie":       []string{cookie},
		"Content-Type": []string{"application/x-www-form-urlencoded;charset=UTF-8"},
...
func (c *Crawler2) extractCourses(ctx context.Context, year, semester string, html []byte) ([]*model.ClassInfoBO, error) {
	...
	doc.Find("li.qz-toolitiplists").Each(func(i int, selection *goquery.Selection) {
		var classInfo model.ClassInfoBO
...
func extractGraduateData(rawJson []byte, stuID, xnm, xqm string) ([]*model.ClassInfoBO, []*model.StudentCourseBO, int, error) {
	...
	infos := make([]*model.ClassInfoBO, 0, length)
	Scs := make([]*model.StudentCourseBO, 0, length)
	sum := v.GetInt("xsxx", "KCMS")

	for _, kb := range kbList.GetArray() {
		// 课程信息
		info := &model.ClassInfoBO{}
