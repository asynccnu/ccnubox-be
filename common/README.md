...
无 DLQ 模式下，坏消息会阻塞对应分区，并且会让整个会话结束：同一组内其他分区会立刻停止并等待下一次会话重建，坏消息一直存在时这些分区只能按退避间隔获得很短的处理窗口。日志提供 `topic`、`partition`、`offset`，不打印包含隐私的消息体。

1. 根据日志定位原始消息，在受控环境读取并按需安全存档；当前实现不提供独立的失败消息存档。
2. 修复生产者、消费兼容逻辑或依赖服务，然后恢复消费。只修复生产者不会改变已经写入 Kafka 的坏消息。
3. 如果必须跳过消息，应先停止消费者，完成存档和业务补偿，并经人工确认后调整消费位点；程序不会自动跳过。
...
| 关键字 | 含义 |
| --- | --- |
| `KAFKA_PARTITION_BLOCKED` | 位点未确认且分区停止消费（坏消息、字段校验失败、依赖长期不可用、classlist 原地重试达到 10 次），必须人工处理 |
| `KAFKA_CONSUME_RETRY` | 本轮处理失败、正在退避重试，通常可自愈；持续出现说明依赖不稳定 |
| `KAFKA_MESSAGE_DROPPED` | 消息被显式丢弃，不会再重投（延迟消息过期、无法入库的木犀计划） |
