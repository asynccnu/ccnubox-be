...
func InitApp() *App {
	...
	classServiceClient := ioc.InitClassClient(clientv3Client, infraConf)
	classHandler := ioc.InitClassHandler(serverConf, classerClient, classServiceClient, logger)
	...
	metricsHandler := ioc.InitMetricsHandler(serverConf, logger, metrics)
	...
	manager := ioc.InitCronxManager(logger, metrics, cmdable)
	app := NewApp(engine, serverConf, v3, tieredScheduler, manager)
	return app
}
